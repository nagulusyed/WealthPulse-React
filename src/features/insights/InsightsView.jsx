  ...
  return (
    <button
      ...
      style={{
        ...
        border: active ? '1.5px solid var(--accent-primary)' : '1.5px solid var(--border-subtle)',
...
// ─────────────────────────────────────────────────────────────
// Tab 1: Monthly Report (merged from ReportsView)
// ─────────────────────────────────────────────────────────────
function MonthlyTab({ privacyMode }) {
  ...
  const report = useMemo(() => {
    ...
    const receivedSettlements = transactions
    ...
    if (totalGrossExpense > 0 && receivedSettlements > 0) {
      ...
      netTransactions = transactions.map((t) =>
        t.type === 'expense' && !isSettlement(t)
          ? { ...t, amount: t.amount * (1 - ratio) }
          : t
      );
    }
    ...
    const totalIncome = netTransactions
      .filter((t) => t.type === 'income' && !isSettlement(t))
      .reduce((s, t) => s + t.amount, 0);
    ...
    const prevGross = prevTxns.filter((t) => t.type === 'expense' && !isSettlement(t)).reduce((s, t) => s + t.amount, 0);
    const prevRecSet = prevTxns.filter((t) => t.type === 'income' && isSettlement(t)).reduce((s, t) => s + t.amount, 0);
    ...
    return { totalSpent, totalIncome, savings, savingsPct, diff, pct, avgDaily, categories, topCat: categories[0] };
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '0.85rem', flexWrap: 'wrap' }}>
        <div className="card" style={{ flex: '1 1 0', minWidth: 0 }}>
          ...
          <div className={blur} style={{ fontSize: '1.15rem', fontWeight: 700, color: 'var(--accent-red)', marginTop: '0.15rem' }}>
            {formatCurrency(report.totalSpent)}
          </div>
          <div style={{ fontSize: '0.7rem', marginTop: '0.25rem', color: report.diff > 0 ? 'var(--accent-red)' : 'var(--accent-green)' }}>
            {report.diff > 0 ? '↑' : '↓'} {Math.abs(report.pct)}% vs last month
          </div>
        </div>
        <div className="card" style={{ flex: '1 1 0', minWidth: 0 }}>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Income</div>
          <div className={blur} style={{ fontSize: '1.15rem', fontWeight: 700, color: 'var(--accent-green)', marginTop: '0.15rem' }}>
            {formatCurrency(report.totalIncome)}
          </div>
          <div style={{ fontSize: '0.7rem', marginTop: '0.25rem', color: 'var(--text-muted)' }}>
            Avg. {formatCurrency(Math.round(report.avgDaily))}/day
          </div>
        </div>
        <div className="card" style={{ flex: '1 1 0', minWidth: 0 }}>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Saved</div>
          <div className={blur} style={{ fontSize: '1.15rem', fontWeight: 700, color: report.savings >= 0 ? 'var(--accent-green)' : 'var(--accent-red)', marginTop: '0.15rem' }}>
            {formatCurrency(Math.abs(report.savings))}
          </div>
          <div style={{ fontSize: '0.7rem', marginTop: '0.25rem', color: 'var(--text-muted)' }}>
            {report.savingsPct}% of income
          </div>
        </div>
      </div>
      ...
      {report.topCat && (
        <InsightCard accent={report.topCat.color}>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
            ...
            {' '}accounts for{' '}
            <strong className={blur}>{formatCurrency(report.topCat.amount)}</strong>
...
function VendorTab({ vendors, privacyMode }) {
  const blur = privacyMode ? 'private-blur' : '';
  if (vendors.length === 0) {
    return <EmptyState emoji="🔍" message="We need at least 2 transactions from the same merchant to recognize patterns. Keep adding transactions!" />;
  }
  return (
    <div>
      <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: 1.5 }}>
        Top merchants by total spend across all your transactions.
      </p>
      {vendors.map((v, i) => (
        <InsightCard key={v.key} accent={v.cat.color}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', minWidth: 0 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem' }}>
                <span style={{ fontWeight: 600, fontSize: '0.88rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textTransform: 'capitalize' }}>
                  {v.displayName}
                </span>
                <span className={blur} style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--accent-red)', flexShrink: 0 }}>
                  {formatCurrency(v.total)}
                </span>
              </div>
              <div style={{ display: 'flex', gap: '0.6rem', marginTop: '0.25rem', fontSize: '0.72rem', color: 'var(--text-muted)', flexWrap: 'wrap' }}>
                <span>{v.count} txn{v.count !== 1 ? 's' : ''}</span>
                <span>·</span>
          ...
          {i === 0 && (
            <div style={{ marginTop: '0.6rem', padding: '0.3rem 0.6rem', background: 'rgba(248,113,113,0.1)', borderRadius: 'var(--radius-sm)', fontSize: '0.73rem', color: '#f87171', display: 'inline-block' }}>
              🏆 Top merchant — consider reviewing this spend
            </div>
          )}
        </InsightCard>
      ))}
...
function BurnRateTab({ burnRate, privacyMode }) {
  const blur = privacyMode ? 'private-blur' : '';
  if (!burnRate) {
    return <EmptyState emoji="📈" message="Burn rate is only available for the current month with at least 3 days of data." />;
  }
  ...
  return (
    <div>
      <InsightCard accent={isOnTrack ? '#2ecc71' : '#e74c3c'}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.75rem' }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: '0.9rem' }}>
              {isOnTrack ? "You're on track!" : 'Budget overage projected'}
            </div>
        ...
        <div style={{ marginBottom: '0.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>
            ...
            <span className={blur}>{formatCurrency(spentSoFar)} / {formatCurrency(totalBudget)}</span>
          </div>
        ...
        {!isOnTrack && (
          <div style={{ marginTop: '0.6rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>
              ...
              <span className={blur} style={{ color: '#e74c3c' }}>{formatCurrency(projectedTotal)}</span>
            </div>
            <div style={{ height: 6, background: 'var(--border-subtle)', borderRadius: 3, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${Math.min(projectedPct, 100)}%`, background: 'rgba(231,76,60,0.5)', borderRadius: 3 }} />
            </div>
          </div>
        )}
      </InsightCard>
      ...
      <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '0.85rem' }}>
        {[
          { label: 'Daily Burn', value: `${formatCurrency(dailyBurn)}/day`, color: 'var(--accent-red)' },
          { label: 'Safe Limit', value: `${formatCurrency(safeDaily)}/day`, color: 'var(--accent-green)' },
          { label: 'Budget Left', value: formatCurrency(Math.abs(budgetLeft)), color: budgetLeft > 0 ? 'var(--accent-green)' : 'var(--accent-red)' },
        ].map((s) => (
      ...
      <InsightCard accent="var(--accent-indigo)">
        <div style={{ fontSize: '0.83rem', lineHeight: 1.65, color: 'var(--text-secondary)' }}>
          {isOnTrack
            ? <>Great discipline! At <span className={blur} style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{formatCurrency(dailyBurn)}/day</span>, you'll finish the month within budget. You can spend up to <span className={blur} style={{ fontWeight: 600, color: 'var(--accent-green)' }}>{formatCurrency(safeDaily)}/day</span>.</>
...
function AnomalyTab({ anomalies, privacyMode }) {
  ...
  if (anomalies.length === 0) {
    return <EmptyState emoji="📉" message="No anomalies detected. Spending patterns look consistent. Needs 2–3 months of history." />;
  }
  const typeConfig = {
    spike: { icon: '🔺', color: '#e74c3c', label: 'Spike', bg: 'rgba(231,76,60,0.1)' },
    drop: { icon: '🔻', color: '#f59e0b', label: 'Drop', bg: 'rgba(245,158,11,0.1)' },
    missing: { icon: '❓', color: '#a78bfa', label: 'Missing', bg: 'rgba(167,139,250,0.1)' },
  };
  return (
    <div>
      <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: 1.5 }}>
        vs your 3-month rolling average per category.
      </p>
      {anomalies.map((a) => {
        ...
        return (
          <InsightCard key={a.catId} accent={cfg.color}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.7rem', minWidth: 0 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: cfg.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', flexShrink: 0 }}>
                {a.cat.emoji}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.3rem' }}>
                  <span style={{ fontWeight: 600, fontSize: '0.88rem' }}>{a.cat.name}</span>
                  <span style={{ background: cfg.bg, color: cfg.color, fontSize: '0.7rem', fontWeight: 700, padding: '0.12rem 0.45rem', borderRadius: 'var(--radius-full)', flexShrink: 0 }}>
                    {cfg.icon} {cfg.label}
                  </span>
                </div>
                <div style={{ display: 'flex', gap: '1rem', marginTop: '0.45rem', flexWrap: 'wrap' }}>
                  {[
                    { label: 'This month', value: formatCurrency(a.currentAmount), color: a.type === 'spike' ? cfg.color : undefined },
                    { label: '3-mo avg', value: formatCurrency(a.avg) },
                    { label: 'Change', value: a.type === 'missing' ? '—' : `${a.pctChange > 0 ? '+' : ''}${a.pctChange}%`, color: cfg.color },
                  ].map((s) => (
                    <div key={s.label}>
                      <div style={{ fontSize: '0.67rem', color: 'var(--text-muted)' }}>{s.label}</div>
                      <div className={blur} style={{ fontWeight: 700, fontSize: '0.9rem', color: s.color }}>{s.value}</div>
                    </div>
                  ))}
                </div>
...
function SubscriptionsTab({ subscriptions, yearlyTotal, privacyMode }) {
  ...
  if (subscriptions.length === 0) {
    return <EmptyState emoji="📆" message="No recurring subscriptions detected yet. We scan for same-amount transactions repeating monthly." />;
  }
    ...
    <div>
      <div className="card" style={{ marginBottom: '0.85rem', background: 'linear-gradient(135deg, rgba(99,102,241,0.12) 0%, rgba(139,92,246,0.08) 100%)', border: '1.5px solid rgba(99,102,241,0.2)', padding: '0.9rem 1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Monthly</div>
            <div className={blur} style={{ fontSize: '1.15rem', fontWeight: 700, color: 'var(--accent-indigo)' }}>{formatCurrency(monthlyTotal)}</div>
          </div>
          <div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Yearly estimate</div>
            <div className={blur} style={{ fontSize: '1.15rem', fontWeight: 700, color: 'var(--accent-violet)' }}>{formatCurrency(yearlyTotal)}</div>
          </div>
        </div>
      </div>
      ...
      {subscriptions.map((sub) => (
        <InsightCard key={sub.key} accent={sub.cat.color}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', minWidth: 0 }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: `${sub.cat.color}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', flexShrink: 0 }}>
              {sub.cat.emoji}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <div style={{ flexShrink: 0, textAlign: 'right' }}>
                  <span className={blur} style={{ fontWeight: 700, fontSize: '0.9rem' }}>{formatCurrency(sub.amount)}</span>
                  <span style={{ fontSize: '0.67rem', color: 'var(--text-muted)' }}>{freqLabel[sub.frequency] || '/mo'}</span>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.25rem', fontSize: '0.7rem', color: 'var(--text-muted)', flexWrap: 'wrap' }}>
                <span>{sub.cat.name}</span>
                <span>·</span>
                <span>{sub.occurrences}× found</span>
                <span>·</span>
                <span className={blur} style={{ color: 'var(--accent-violet)' }}>{formatCurrency(sub.yearlyEstimate)}/yr</span>
              </div>
            </div>
          </div>
        </InsightCard>
      ...
      {!hasData ? (
        <div className="card" style={{ textAlign: 'center', padding: '3rem 1rem' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
            Add some transactions and this page will come to life.
          </p>
        </div>
      ) : (
        <>
          {/* Tab strip — scrollable, never wraps */}
          <div style={{
            display: 'flex',
            gap: '0.45rem',
            overflowX: 'auto',
